/* In this SQL project, I analyze my own Netflix data to gain insights into my viewing habits,
such as which shows or movies I watch the most, what genres I prefer, and when I tend to watch Netflix the most.
I also explore patterns in my viewing behavior, such as binge-watching habits and the average length of my viewing sessions. */

--Skills used: CASE, CTE's, WINDOW FUNCTIONS, AGGREGATE FUNCTIONS, CONVERTING DATA TYPES,
...
-- extracting everything before the first colon without including the colon itself, in the title column/string 
...
UPDATE netflixviewinghistory
SET EpisodeName = CASE WHEN EpisodeName = 'IT’S GOOD TO BE BACK ON THE MOON' THEN 'IT''S GOOD TO BE BACK ON THE MOON'
                       WHEN EpisodeName = 'Piña Colada' THEN 'Pina Colada'
					   ELSE EpisodeName
					   END ;
...
SELECT Genre,GenreCount FROM (
  SELECT Genre, COUNT(Genre) AS GenreCount,ROW_NUMBER() OVER (ORDER BY COUNT(Genre) DESC) AS rn
  FROM netflixviewinghistory
  GROUP BY Genre
)
FavGenre
...
-- Average number of shows I watch a day (3)
WITH ShowAVG AS (
...
-- Average number of movies I watch a day (1)
WITH MovieAVG AS (
  SELECT Date AS Viewing_Date, COUNT(Title) as TitleTotal
  FROM netflixviewinghistory
  WHERE Category = 'Movie'
